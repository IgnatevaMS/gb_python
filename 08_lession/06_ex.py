...
if __name__ == '__main__':
    storage = Storage()
    storage.add(Printer.create(4, vendor="HP", model="LaserJet2000"))
    storage.add(Scanner.create(3, vendor="MSI", model="3648"))
    storage.add(Scanner.create(2, vendor="Cuosera", model="32154"))
    storage.add(Xerox.create(6, vendor="Xerox", model="F123"))
    print(storage)

    for idx, itm in storage.filter_by(department=None, vendor="MSI", model="3648"):
        print(f"Резервируем {itm} в 'Отдел Бухгалтерии'")
        storage.transfer(idx, 'Отдел Бухгалтерии')
